...
Points[Pdict['KobeBryant'],Sdict['2005']]
...
def myplot(data, playerlist=Players):
    Col={"KobeBryant":"Black","JoeJohnson":"Green","LeBronJames":"Blue","CarmeloAnthony":"Yellow","DwightHoward":"Red","ChrisBosh":"Magenta","ChrisPaul":"Purple","KevinDurant":"Orange","DerrickRose":"Brown","DwayneWade":"Pink"}
    for names in playerlist:
        plt.plot(data[Pdict[names]], c=Col[names],ls='--',marker='s',ms=7, label=names)
    plt.legend(loc='upper left',bbox_to_anchor=(1,1))
    plt.xticks(list(range(0,10)),Seasons, rotation="vertical")
    plt.xlabel('Seasons')
    plt.ylabel('')
...
myplot(Games,{"KobeBryant","JoeJohnson","LeBronJames","CarmeloAnthony"})
...
# Kobe Bryantt and Derrick Rose graph abnormal because of injury
...
# Kevin Durantt graph increasing by years
...
myplot(np.matrix.round(FreeThrows/FreeThrowsAttempts,2)*100 )
